    ...
    public sealed class FramedGlowMask
    {
        ...
        public FramedGlowMask(string asset, int frameWidth, int frameHeight, bool pretendEveryFrameHaveGlow = false)
        {
            ...
            else
            {
                ...
                {
                    var colData = Texture.GetColorsFromTexture(); // Get the color data from the texture (1D array)
                    ...
                    Parallel.For(0, FrameXCount * FrameYCount, (i) =>
                    {
                        ...
                        for (int x = xStart; x < xEnd; x++)
                        {
                            ...
                            {
                                // Convert 2D (x, y) coordinates into 1D index for colData
                                int index = y * Texture.Width + x;  // Assuming 'Texture.Width' is the width of the entire texture

                                Color col = colData[index];  // Access color from the 1D array
                        ...
                        _HasGlowContent[xFrame, yFrame] = frameHasData; // Store whether the frame has glow content
                    });
                });
            }
        }
